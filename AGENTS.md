...
- `ccache` cannot write to `/home/martin/.cache/ccache` in this environment (read-only filesystem). When compiling, run `meson compile` with `CCACHE_DISABLE=1` to avoid build failures.

# Session Lessons

- In `PlayerController.set_play_state`, avoid `Gdk.threads_add_idle` for UI state updates; it introduced lifetime crashes. The polling timer already runs on the main loop, so update synchronously.
- Map `Gst.State.READY` to a non-stopped UI state (buffering) in `GstStreamPlayer.update_play_state` to prevent Play icon flicker on stations that bounce READY/PAUSED.
- If a station flaps `PLAYING ↔ PAUSED/READY`, debounce the UI state in `PlayerController.update_player_state` (short window after PLAYING) to keep the icon stable.

# Documentation & Formatting

- Keep Doxygen tags accurate and scoped: avoid `@class` on constructors; use concise `@brief` for methods and keep it aligned with current behavior.
- Keep comments truthful: update or remove stale explanations (e.g., references to `threads_add_idle` when code is synchronous).
- Match existing brace placement in this repo (class/method opening brace on the next line, `switch`/`case` formatting as in file).
- Keep inline comments aligned with surrounding indentation; avoid awkward mid‑indent comments inside signal callbacks.
